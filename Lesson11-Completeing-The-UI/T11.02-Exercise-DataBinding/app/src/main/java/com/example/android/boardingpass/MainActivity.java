...
public class MainActivity extends AppCompatActivity {
    ...
    private void displayBoardingPassInfo(BoardingPassInfo info) {
        ...
        String boardingTimeStr = dateFormat.format(info.boardingTime.getTime());
        mBinding.textViewBoardingTime.setText(boardingTimeStr);

        String departureTimeStr = dateFormat.format(info.departureTime.getTime());
        mBinding.textViewDepartureTime.setText(departureTimeStr);

        String arrivalTimeStr = dateFormat.format(info.arrivalTime.getTime());
        mBinding.textViewArrivalTime.setText(arrivalTimeStr);

        long hours = TimeUnit.MINUTES.toHours(info.getMinutesUntilBoarding());
        long minutes = info.getMinutesUntilBoarding()-TimeUnit.HOURS.toMinutes(hours);
        String boardingInStr = String.format("%02d:%02d", hours, minutes);
        mBinding.textViewBoardingInCountdown.setText(boardingInStr);
